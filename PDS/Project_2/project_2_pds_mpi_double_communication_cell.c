...
//GET ELEMENTS FROM TEXT IN ROOT PROCESS//
        ...
        int prev,next,corpusdim,new_corpusdim;
        corpusdim = loc[rank];
        if (rank==0) {
                prev = size-1;
                next = rank+1;
        ...
        double *c1,*c2;
        int *id1,*id2;
        id1= (int*)malloc(sizeof(double)*N);
        id1 = MY_ID;
        id2= (int*)malloc(sizeof(double)*N);
        c1 = (double*)malloc(sizeof(double)*N*D);
        c1 = QUERRY;
        c2 = (double*)malloc(sizeof(double)*N*D);
        double start,end;
        if (rank==0) {
        ...
        MPI_Irecv(c2,N*D,MPI_DOUBLE,prev,prev,MPI_COMM_WORLD, &req[1]);
        MPI_Isend(id1,N,MPI_INT,next,rank,MPI_COMM_WORLD, &req[0]);
        MPI_Irecv(id2,N,MPI_INT,prev,prev,MPI_COMM_WORLD, &req[1]);
        kNN(QUERRY,c1,MY_ID,id1,N,D,k,*k1);
        ...
        for (int itt=0; itt<size-1; itt++)
        {
                ...
                if (itt%2!=0) {
                        ...
                        MPI_Waitall(2,req,MPI_STATUSES_IGNORE);
                        MPI_Isend(c2,N*D,MPI_DOUBLE,next,rank,MPI_COMM_WORLD,&req[0]);
                        MPI_Irecv(c1,N*D,MPI_DOUBLE,prev,prev,MPI_COMM_WORLD,&req[1]);
                        MPI_Isend(id2,N,MPI_INT,next,rank,MPI_COMM_WORLD, &req[0]);
                        MPI_Irecv(id1,N,MPI_INT,prev,prev,MPI_COMM_WORLD, &req[1]);

                        kNN(QUERRY,c2,MY_ID,id2,N,D,k,*k1);
                        if (rank==0) {
                                printf("%d\n",itt );
                        }
